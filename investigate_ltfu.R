...
dat_matching_84_2weeks_2nd_dose <- dat_infected_lc %>%
...
a <- dat_matching_84_2weeks_2nd_dose

a$asked_lc <- ifelse(!is.na(a$lc_last_question_date) &
                       a$lc_last_question_date >= as.numeric(a$infection_date), 1, 0)

table(a$asked_lc, useNA="ifany")
sum(a$asked_lc) / nrow(a)

a$age10 <- NA
a$age10[a$age_at_visit>=18 & a$age_at_visit<=29] <- "1"
a$age10[a$age_at_visit>=30 & a$age_at_visit<=39] <- "2"
a$age10[a$age_at_visit>=40 & a$age_at_visit<=49] <- "3"
a$age10[a$age_at_visit>=50 & a$age_at_visit<=59] <- "4"
a$age10[a$age_at_visit>=60 & a$age_at_visit<=69] <- "5"
...
dists <- cov.dist.cat(
  vars = c("age10", "sex", "white", "gor9d", "imd_quintile", "health_status"),
  dataset = a,
  exposure = "asked_lc"
)
